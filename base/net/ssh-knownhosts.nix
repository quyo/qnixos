...

{
  ...
  programs.ssh.knownHosts =
    let
      hostkey = host: builtins.readFile "${secrets}/secrets/hosts/${host}/ssh/ssh_host_ed25519_key.pub";
    in
    {
      github-ed25519 = {
      ...
      "ka-sky-harbor.tailaccaf.ts.net".publicKey = hostkey "harbor";

      ka-sky-nyx = {
        hostNames = [ "ka-sky-nyx.tailaccaf.ts.net" "nyx.sky.quyo.net" "sky.quyo.net" ];
        publicKey = hostkey "nyx";
      };

      "192.168.72.102".publicKey = hostkey "bigben";
    };
